...
/**
 * AWS X-Ray distributed tracing configuration for workers.
 * 
 * This configuration:
 * - Enables X-Ray tracing for SQS message processing
 * - Configures sampling rules to control trace collection rate
 * - Registers EC2 and EKS plugins for metadata enrichment
 * - Enables automatic instrumentation of AWS SDK v2 clients (S3, SQS, Textract, Bedrock, SES)
 * 
...
public class XRayConfig {
    ...
    @Value("${spring.application.name:food-cost-calculator-workers}")
     ...
     * - Sampling rules: controls which operations to trace
     * 
